...
export default function Card({ dark, white, data, id, className, noStripe }) {
  ...
  return (
    <Motion
      ...
      initial='down'
      className={cn(
        'max-w-lg px-4 overflow-hidden w-full cursor-pointer group rounded-lg shrink-0 bg-zinc-100',
        { 'bg-secondary': dark, 'bg-white': white },
        className
      )}
    >
      <div className='overflow-hidden scale-x-110'>
        <Image
          className={cn(
            'object-cover w-full  group-hover:scale-110 transition-transform duration-[2s] h-80 border-b-[10px] border-primary',
            { 'border-0': noStripe }
          )}
          src={
